...
typedef struct node{
    Cidade city;
    struct node* nextPopulacao;
    struct node* prevPopulacao;
    struct node* nextAltitude;
    struct node* prevAltitude;
...
Cidade criarCidade(char nome[100], float temp_media, int populacao, int altitude){
    Cidade aux;
...
void imprimeListaPopulacao(Lista* li){
    if(listaVazia(li) != 1){
        Node *aux = li->firstPopulacao;
        printf("Em ordem crescente de poopulação temos: \n");
        while(aux != NULL){
...
void imprimeListaTemp(Lista* li){
    if(listaVazia(li) != 1){
        Node *aux = li->firstTemp;
        printf("Em ordem crescente de Temperatura temos: \n");
        while(aux != NULL){
            imprimeCidade(aux->city);
            aux  = aux->nextTemp;
        }
...
void esvaziaLista(Lista* li){
    if(listaVazia(li) != 1){
        Node** vet = (Node**)malloc(1000*sizeof(Node*));
        int index = 0;
        Node* aux = li->firstAltitude;
        while(aux!=NULL){
            *(vet + index) = aux;
